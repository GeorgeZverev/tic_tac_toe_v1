...
def is_free_of_enemys_token(combined_list: [], proposed_coordinate: str) -> bool:
...
def is_not_bad_coord(board: [], valid_coordinates: [], proposed_coordinate: str, token: str) -> bool:
    ...
    if is_free_of_enemys_token(combined_list, proposed_coordinate):
        return True
    return False
    ...
    for triple in right_coordinates:
        if triple[0] != '.' and triple.count(triple[0]) == len(triple):
            if triple[0] == 'x':
                ...
                return 2
    if is_free_square_present(board) == False:
        return 3
    return 0
...
def play_round_for_computer(board: [], coordinate: str, token: str, valid_coordinates: []):
    success = play_computer_round(board, token, valid_coordinates)
...
if __name__ == '__main__':
    ...
    attempts = 0
    mode, flag = accept_mode()
    while (True):
        if flag ==False :
            print('You chose a non-existent mod ')
            break
